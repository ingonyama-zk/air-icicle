...
pub fn generate_trace_rows<F: FieldImpl + Arithmetic>(
    ...
    let mut long_trace = vec![F::zero(); trace_length << extra_capacity_bits];
...
fn generate_trace_rows_for_perm<F: FieldImpl + Arithmetic>(
    ...
    rows[0].a = initial_state.clone();
    rows[0].preimage = initial_state.clone();

    generate_trace_row_for_round(&mut rows[0], 0, &mut current_state);

    for round in 1..rows.len() {
        rows[round].preimage = initial_state.clone();
